...
WeaponComponent::WeaponComponent() : m_Damage(20.0f), m_FireRate(0.5f), m_Ammo(30), m_MaxAmmo(30), m_LastFireTime(0.0f), m_ShotLineTimer(0.0f) {}
...
void WeaponComponent::Update(float deltaTime) {
    ...
    if (input.IsMouseButtonPressed(MouseButton::Left) && m_Ammo > 0 && deltaTime - m_LastFireTime >= m_FireRate) {
        ...
        Vector3 target = origin + direction * 10.0f;
        ...
        for (auto& e : entity->GetScene()->GetEntities()) {
            if (auto* alien = dynamic_cast<AlienCharacter*>(e.get())) {
                ...
                if (dist < 10.0f) { // Assume hit if within 10 units
